...
const seedReceivedRequests = async () => {
  await connectDB();
  await seedUsers();
  ...
  const pairs = [
    [users[1]._id, users[0]._id],
    [users[2]._id, users[0]._id],
    [users[3]._id, users[0]._id],
    [users[0]._id, users[1]._id],
    [users[2]._id, users[1]._id],
    [users[3]._id, users[1]._id],
  ];
  ...
  console.log("Seeded sample received requests successfully");
  await mongoose.disconnect();
};

seedReceivedRequests().catch((err) => {
  console.error(err);
  process.exit(1);
});
